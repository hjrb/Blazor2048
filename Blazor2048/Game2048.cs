...
public class Game2048
{
    ...
    public IEnumerable<Moves> CanMove((int row, int column) t) => this.CanMove(t.row, t.column);
    public IEnumerable<Moves> CanMove(int index) => this.CanMove(ToGrid(index));
    /// <summary>
    /// enumerates if an how a cell (given by row and column index) can move
    /// if not move is possible an empty iteration will be returned
    /// </summary>
    /// <param name="row"></param>
    /// <param name="column"></param>
    /// <returns></returns>
    public IEnumerable<Moves> CanMove(int row, int column)
    {
        if (this[row, column] == 0) yield break;

        bool Test(int otherrow, int othercolumn)
        {
            return otherrow >= 0 && otherrow < Size && othercolumn >= 0 && othercolumn < Size
                    && (this[otherrow, othercolumn] == 0 || this[otherrow, othercolumn] == this[row, column]);
        }
        if (Test(row - 1, column)) yield return Moves.Down;
        if (Test(row + 1, column)) yield return Moves.Up;
        if (Test(row, column - 1)) yield return Moves.Left;
        if (Test(row, column + 1)) yield return Moves.Right;

    }

    /// <summary>
    /// convert an index into a tuple (row, column)
    /// <param name="index"></param>
    /// <returns></returns>
    public (int row, int column) ToGrid(int index) => (index / Size, index % Size);

    /// <summary>
    /// convert a row,column tuple to an index in the array
    /// </summary>
    /// </summary>
    /// <param name="row"></param>
    /// <param name="column"></param>
    /// <returns></returns>
    public int ToIndex(int row, int column) => row * Size + column;
    ...
    public IEnumerable<int> Where(CellSelector cond)
    {
        List<int> list = new();
        Parallel.ForEach(Cells.Select((val, idx) => new { val, idx}), tuple =>
                {
                    if (cond(tuple.val, tuple.idx)) lock (list) { list.Add(tuple.idx); };
                });
        return list;
    }
    ...
    /// <summary>
    /// return true if any cell has a value of 2048 or higher
    /// </summary>
    /// <returns></returns>
    public bool Won() => this.Max >= 2048;
    /// <summary>
    /// returns true if no more moves are possible. This can be the case if won or not won!
    /// </summary>
    /// <returns></returns>
    public bool Done() => !this.Where((_, index) => CanMove(index).Any()).Any();

    /// <summary>
    /// a private random instance
    /// </summary>
